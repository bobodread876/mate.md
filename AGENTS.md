...
- Reference impl: **complete and green** — parse / normalize / validate / verify
  (Ed25519 + BIP-340) / sign / keygen / Nostr transport / CLI. Build clean, 35
  tests pass, 23/23 fixtures pass.
- CLI proof lifecycle (Unreleased): `mate keygen` / `mate sign` / `mate verify`
  now produce + check Ed25519 proofs (v0.2.0 was verify-only).
- Nostr transport (Unreleased): `mate keygen --nostr` / `mate nostr-publish` /
  `mate nostr-resolve` implement `docs/extension-nostr.md` (draft NIP-BD) — live
  publish/resolve of kind 30317 + 1317 over relays, zero new deps. NIP-BD is a
  draft branch on `bobodread876/nips` (`nip-agent-bonds`).
