...
export function normalizeMateDocument(data: MateDocument): string {
...
function normalizeValue(key: string, value: unknown): CanonicalValue | undefined {
  ...
  if (typeof value === 'number') {
    if (!Number.isInteger(value)) {
      throw new Error(`Canonical MATE.md JSON forbids non-integer number at ${key}`);
    }
    return value;
  }
...
function normalizeRecord(
  ...
  const keys = options.topLevel
    ? topLevelKeys(record)
    : Object.keys(record).sort(compareCodePoints);

  for (const key of keys) {
    if (options.omitProofs && key === 'proofs') {
      continue;
    }

    const value = normalizeValue(key, record[key]);
    if (value !== undefined) {
      output[key.normalize('NFC')] = value;
    }
  }
